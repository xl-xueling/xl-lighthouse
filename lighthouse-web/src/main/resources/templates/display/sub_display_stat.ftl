...
<div class="row nested-sub-page">
  <div class="col-md-12">
    <section class="invoice" style="padding: 5px 10px;margin: 2px 0px;border: none;">
      <div class="row">
        <div class="col-xs-12">
          <h2 class="page-header" style="border-bottom:none;margin: 10px 0 5px 0;padding-bottom:3px;font-size:18px;">
            ...
            <#if statEntity.templateEntity.limitFlag?string("true","false") == "true">
              <span style="font-size: 14px;margin-left: 10px;">
                    <label>
                      <input type="checkbox" name="limitType" class="minimal" onclick="DISPLAY_OPERATOR.changeLimitType('${statEntity.id}','${startDate}','${endDate}','1','true')">
                    </label>
                    Limit
                </span>
            </#if>
          ...
          <div class="box-body">
            <div class="box box-primary" style="border-top:2px solid #8e9dd1">
              <div class="layui-form box-header">
                ...
                <#if (projectEntity.privilegeIds?seq_contains(5) && statEntity.builtIn?string("true","false") == "false")>
                            ...
                            <li>
                                <span>
                                  ...
                                  <#if statEntity.displayType == displayTypeEnum.displayType>
                                    <a href="javascript:DISPLAY_OPERATOR.updateDisplayType('${statEntity.id}','${displayTypeEnum.displayType}','${displayTypeEnum.chartName}','true')"><i class="fa fa-fw fa-dot-circle-o"></i></a>
                                  <#else>
                                    <a href="javascript:DISPLAY_OPERATOR.updateDisplayType('${statEntity.id}','${displayTypeEnum.displayType}','${displayTypeEnum.chartName}','true')"><i class="fa fa-fw fa-circle-o"></i></a>
                                  </#if>
                                </span>
              ...
              <div class="box-body">
                <#if (statEntity.templateEntity.statStateList?size gt 1)>
                 <div class="ldp-stat-chart" statId="${statEntity.id}" stateIndex="-1" style="height: 300px;padding: 10px;"></div>
                <#else>
...
<@baseFrame.sub_tail />
<script>
  var statId = '${statEntity.id}';
  var startDate = '${startDate}';
  var endDate = '${endDate}';
  var filterParams = JSON.stringify(${filterParams});
  var isSub = 'true';
  function _SubPageLoadCallBack(){
    FILTER_OPERATOR.initFilter(filterParams,'true');
    DISPLAY_OPERATOR.dateSwitch(statId,'true');
