...
public class BizOrderStatSample implements SimulationModalSample<HashMap<String,Object>>{
    ...
    public HashMap<String, Object> generateSample() throws Exception {
        ...
        paramMap.put("paymode",ThreadLocalRandom.current().nextInt(1,6));
        paramMap.put("version",ThreadLocalRandom.current().nextInt(1,4) + "." + ThreadLocalRandom.current().nextInt(1,5) + "." + ThreadLocalRandom.current().nextInt(1,5));
        return paramMap;
    }
}
