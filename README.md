### **一键部署，一行代码接入，轻松实现海量数据实时统计，使用XL-LightHouse后：
* 1、再也不需要用FlinkSQL、SparkSQL、ClickHouse这种臃肿笨重的方案跑数了；
* 2、再也不需要疲于应付各种反复琐碎、对个人价值提升又无多大益处的数据统计需求了；
* 3、轻松帮您实现任意细粒度的监控指标，是监控服务运行状况，排查各类业务数据波动、指标异常类问题的好帮手；
...
* XL-LightHouse项目架构支持超大规模的数据量和高并发数据写入，可以支撑一线互联网大厂的数据量。
...
* 1、同一个测试账号被多人同时访问可能会异常退出，您可以使用：admin、test1、test2、test3、test4、test5以及admin1、admin2、admin3 ... admin99 等100多个测试账号中的任何一个登录访问，密码均为123456。
