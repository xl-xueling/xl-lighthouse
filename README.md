...
有些时候对数据指标的需求，往往只在某个特定阶段。比如：新接口上线要进行接口的压力测试、性能优化；线上业务出现数据异常问题需要排查；数据库读写压力突然暴涨，需要确定异常请求的来源等等，
对于此类场景，流式统计可以起到至关重要的作用。但这类场景一般不需要持续很长时间，可能两三周甚至两三天。这种情况可以使用单机版本。一键部署，轻量级使用，问题排查完，将XL-LightHouse删除即可，灵活的使用XL-LightHouse可以为您解决很多棘手的问题！
...
-  [开源版本（最新：v2.3.7）](https://dtstep.com/docs/110027/)
-  [商业版本（最新：v2.3.7-pro.1）](https://dtstep.com/docs/110072/)
