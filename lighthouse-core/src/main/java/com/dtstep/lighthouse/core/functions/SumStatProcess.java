...
public final class SumStatProcess extends StatProcess<Pair<String,Long>>{

    private static final Logger logger = LoggerFactory.getLogger(SumStatProcess.class);

    private static final long serialVersionUID = 4471275218046965810L;

    private static final int poolSlotSize = StatConst.DEFAULT_POOL_SLOT_SIZE;

    private static final EventPool<MicroBucket> eventPool = new BlockingEventPool<>("SumStorageEventPool",poolSlotSize,300000);

    public SumStatProcess(StatExtEntity statExtEntity, String metaName, String aggregateKey, String dimensValue){
    ...
    static {
        ...
        ScheduledExecutorService service = ScheduledThreadPoolBuilder.
                newScheduledThreadPoolExecutor(threadSize,new BasicThreadFactory.Builder().namingPattern("sum-consumer-schedule-pool-%d").daemon(true).build());
    ...
    public void evaluate(StatState statState, List<Pair<String,Long>> messageList, long batchTime) throws Exception {
        ...
        if(logger.isTraceEnabled()){
            logger.trace("lighthouse trace,sum evaluate,stat:{},formula:{},dimens:{},rowKey:{},message size:{},cost:{}"
