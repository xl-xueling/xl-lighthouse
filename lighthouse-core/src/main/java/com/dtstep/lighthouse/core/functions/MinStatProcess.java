...
public final class MinStatProcess extends StatProcess<Pair<String,Long>>{
    ...
    private static final EventPool<MicroBucket> eventPool = new BlockingEventPool<>("MinStorageEventPool",poolSlotSize,300000);

    private final static Cache<String, Long> MIN_CACHE = Caffeine.newBuilder()
    ...
    static {
        ...
        ScheduledExecutorService service = ScheduledThreadPoolBuilder.
                newScheduledThreadPoolExecutor(threadSize,new BasicThreadFactory.Builder().namingPattern("min-consumer-schedule-pool-%d").daemon(true).build());
    ...
    public void evaluate(StatState statState, List<Pair<String,Long>> messageList, long batchTime) throws Exception {
        ...
        long result = messageList.parallelStream().map(x -> {
            ...
            return FormulaCalculate.calculate(statState,envMap,batchTime); })
                .filter(x -> x != NIL_VAL && x != ILLEGAL_VAL).mapToLong(x->x).min().orElse(0);
        Long cacheVar = MIN_CACHE.getIfPresent(aggregateKey);
        if(result == 0L || (cacheVar != null && result >= cacheVar)){
            return;
        }
        MIN_CACHE.put(aggregateKey,result);
        MicroBucket microBucket = new MicroBucket.Builder()
                ...
                .setCalculateEnum(MicroCalculateEnum.MinCalculate)
        ...
        if(logger.isTraceEnabled()){
            logger.trace("lighthouse trace,min evaluate,stat:{},formula:{},dimens:{},rowKey:{},message size:{},cost:{}"
