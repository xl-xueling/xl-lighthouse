...
public final class BitCountStatProcess extends StatProcess<Pair<String,Long>> {
    ...
    static {
        ...
        ScheduledExecutorService service = ScheduledThreadPoolBuilder.
                newScheduledThreadPoolExecutor(threadSize,new BasicThreadFactory.Builder().namingPattern("bitcount-consumer-schedule-pool-%d").daemon(true).build());
    ...
    public void evaluate(StatState statState, List<Pair<String,Long>> messageList, long batchTime) throws Exception {
        ...
        long expireSeconds = (statExtEntity.getTimeUnit() == TimeUnit.DAYS) ? TimeUnit.DAYS.toSeconds(2) : (statExtEntity.getTimeUnit() == TimeUnit.HOURS
                ? TimeUnit.HOURS.toSeconds(2) : TimeUnit.HOURS.toSeconds(1));
